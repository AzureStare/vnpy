...
运行方式（推荐在 docker-compose 的 app 容器内执行，自动使用 DATABASE_* 连接 db）：
  python flagship/scripts/sync_ref_tickers_polygon.py --market stocks --locale us --type CS --active
...
from flagship.scripts.pg_ticker_db import create_ticker_tables, get_pg_connection
